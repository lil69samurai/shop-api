...
const CreateOrderPage = () => {
  ...
  return (
    <div className="bg-stone-50 min-h-screen pb-12"><div className="max-w-3xl mx-auto pt-6 sm:pt-8 px-4 sm:px-6">
      {/* Step indicator */}
      <div className="flex items-center justify-center mb-6 sm:mb-8">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className={"w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-xs sm:text-sm font-bold " + (step >= 1 ? "bg-amber-500 text-white" : "bg-stone-200 text-stone-400")}>1</div>
          <span className={"text-xs sm:text-sm font-medium " + (step >= 1 ? "text-slate-800" : "text-stone-400")}>{t("order.step1")}</span>
          <div className="w-8 sm:w-12 h-0.5 bg-stone-300"></div>
          <div className={"w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-xs sm:text-sm font-bold " + (step >= 2 ? "bg-amber-500 text-white" : "bg-stone-200 text-stone-400")}>2</div>
          <span className={"text-xs sm:text-sm font-medium " + (step >= 2 ? "text-slate-800" : "text-stone-400")}>{t("order.step2")}</span>
        </div>
      </div>
      ...
      {step === 1 && (<div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-stone-100 p-4 sm:p-6">
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3">{paymentMethods.map((pm) => (
            <button key={pm.value} type="button" onClick={() => { setForm(prev => ({ ...prev, paymentMethod: pm.value })); if (formErrors.paymentMethod) setFormErrors(prev => ({ ...prev, paymentMethod: "" })); }}
              className={"p-3 sm:p-4 border-2 rounded-xl text-left transition-all " + (form.paymentMethod === pm.value ? "border-amber-500 bg-amber-50 shadow-sm" : "border-stone-200 hover:border-stone-300")}>
              <div className="text-xl sm:text-2xl mb-1">{pm.icon}</div>
              <div className="font-bold text-xs sm:text-sm text-slate-800">{pm.label}</div>
              {pm.desc && <div className="text-xs text-stone-500 mt-0.5 hidden sm:block">{pm.desc}</div>}</button>))}</div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          <Link to="/cart" className="flex-1 text-center bg-stone-200 text-stone-700 py-3 rounded-lg text-base sm:text-lg font-bold hover:bg-stone-300 transition">← {t("order.backToCart")}</Link>
          <button onClick={handleNextStep} className="flex-1 bg-amber-500 text-white py-3 rounded-lg text-base sm:text-lg font-bold hover:bg-amber-400 transition">{t("order.confirmOrder")} →</button></div>
      </div>)}
      ...
      {step === 2 && (<div className="space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-stone-100 p-4 sm:p-6">
          ...
          <div className="mb-6"><h3 className="text-sm font-bold text-amber-600 mb-3">{t("order.shippingDest")}</h3>
            <div className="bg-stone-50 rounded-lg p-3 sm:p-4 space-y-1 text-sm">
              ...
              <p><span className="text-stone-500">{t("order.addressLabel")}:</span> <span className="font-medium text-slate-800 break-all">{form.address}</span></p>
          ...
          <div className="mb-6"><h3 className="text-sm font-bold text-amber-600 mb-3">{t("order.payMethod")}</h3>
            <div className="bg-stone-50 rounded-lg p-3 sm:p-4 flex items-center gap-3"><span className="text-2xl">{selectedPayment?.icon}</span>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-stone-100 p-4 sm:p-6"><div className="space-y-2">
          ...
          <div className="border-t border-stone-200 pt-3 mt-3 flex justify-between items-center">
            <span className="text-base sm:text-lg font-bold text-slate-800">{t("order.totalPayment")}</span>
            <span className="text-xl sm:text-2xl font-bold text-amber-600">¥{finalTotal.toFixed(0)}</span></div></div></div>

        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          <button onClick={() => { setStep(1); window.scrollTo(0, 0); }} className="flex-1 text-center bg-stone-200 text-stone-700 py-3 rounded-lg text-base sm:text-lg font-bold hover:bg-stone-300 transition">← {t("order.goBack")}</button>
          <button onClick={handleSubmit} disabled={submitting} className="flex-1 bg-amber-500 text-white py-3 rounded-lg text-base sm:text-lg font-bold hover:bg-amber-400 disabled:bg-stone-300 disabled:cursor-not-allowed transition">
