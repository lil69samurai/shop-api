...
const HomePage = () => {
  ...
  const [heroImages, setHeroImages] = useState(BANNER_URLS);
  const [currentHero, setCurrentHero] = useState(0);
  const [categoryImages, setCategoryImages] = useState({});

  useEffect(() => {
    const validateBanners = async () => {
      try {
        ...
        console.log("[Hero] Valid banners:", validBanners.length, validBanners);
        if (validBanners.length > 0) {
          setHeroImages(validBanners);
        } else {
          setHeroImages(BANNER_URLS);
        }
      } catch (e) {
        console.log("[Hero] Banner check failed", e);
        setHeroImages(BANNER_URLS);
      }
    };
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          console.log("[Hero] Products fetch failed", e);
        }
        ...
        products.forEach((p) => {
          ...
          const image = getImageSrc(p.imageUrl);
          if (catId && !catImgMap[catId] && image) {
            catImgMap[catId] = image;
          }
        });
        ...
        products.forEach((p) => {
          const image = getImageSrc(p.imageUrl);
          if (image) productImgs.push(image);
        });
        ...
        try {
          ...
          console.log("[Home] Categories fetch failed", e);
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (currentHero >= heroImages.length && heroImages.length > 0) {
      setCurrentHero(0);
    }
  ...
  return (
    <div className="bg-stone-50">
      {/* Hero */}
      <div className="relative h-[420px] sm:h-[500px] md:h-[600px] overflow-hidden bg-slate-900">
        {heroImages.length > 0 ? (
        ...
        <div className="relative z-10 h-full flex flex-col justify-center items-center text-center px-4 sm:px-6">
          <p className="text-amber-400 text-xs sm:text-sm font-medium tracking-widest mb-3 sm:mb-4">
            {t("home.brandSub")}
          </p>
          <h1 className="text-3xl sm:text-4xl md:text-6xl font-extrabold text-white mb-3 sm:mb-4 tracking-tight drop-shadow-lg">
            {t("home.heroTitle")}
          </h1>
          <p className="text-sm sm:text-base md:text-lg text-slate-200 mb-8 sm:mb-10 max-w-2xl mx-auto leading-relaxed drop-shadow px-2">
            {t("home.heroDesc")}
          </p>
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto px-6 sm:px-0">
            <Link
              to="/products"
              className="bg-amber-500 text-slate-900 font-bold px-8 py-3 rounded-lg text-base sm:text-lg hover:bg-amber-400 transition shadow-lg text-center"
            >
              {t("home.shopNow")}
            </Link>
            <Link
              to="/register"
              className="border-2 border-white text-white font-semibold px-8 py-3 rounded-lg text-base sm:text-lg hover:bg-white hover:text-slate-900 transition text-center"
            >
              {t("home.registerCta")}
            </Link>
          </div>
      ...
      <div className={"py-12 sm:py-16 " + (Array.isArray(categories) && categories.length > 0 ? "bg-stone-50" : "bg-white")}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex justify-between items-center mb-6 sm:mb-8">
            ...
            <Link to="/products" className="text-amber-600 hover:text-amber-700 font-medium text-sm sm:text-base">
              {t("home.viewAll")}
            </Link>
          </div>
      ...
      <div className="bg-slate-900 text-white py-12 sm:py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center">
          ...
          <Link
            to="/register"
            className="inline-block bg-amber-500 text-slate-900 font-bold px-8 py-3 rounded-lg text-base sm:text-lg hover:bg-amber-400 transition"
          >
            {t("home.ctaButton")}
          </Link>
        </div>
